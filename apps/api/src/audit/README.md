...
First consumer: `RmmController.getRemoteControl` (`apps/api/src/rmm/rmm.controller.ts`) logs `rmm.remote_control.requested` every time a MeshCentral control URL is issued. This is the honest limit of what Elyzian can audit for that flow — it records that a session was *granted*, not what happened inside it (MeshCentral has its own session log, outside Elyzian's visibility).
